...
\begin{document}
  ...
  states. When $F$ is the the bag functor, a state $x$ can reach
...
%%   We start by giving multiple plausible definitions of finite bag
...
%%   definition which admits a final coalgebra built as the
...
%%   conclude by comparing these definitions and their final coalgebras.
  ...
  is no longer inductively defined as in Martin-L{\"o}f Type Type (MLTT).
  ...
  methatheory) is problematic. The first step in the construction
  ...
  \begin{theorem}\label{thm:InjPresImpliesLLPO}
    The function %limit-preservation map 
  ...
  \begin{theorem}
    For any type $X$, there is an equivalence $\SetTrunc{\FMGpd X} ≃
    \FinSet X$.
  \end{theorem}
