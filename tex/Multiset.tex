...
\title{The Final Coalgebra of the Finite Bag Functor in Homotopy Type Theory}
\author{%
...
Finally, we introduce the type of finite bags over $X$ as an \emph{analytic functor}~\cite{Joyal1986}, which is provably equivalent to $\M X$, $\SetQuot[\List X][\Perm]$ and $\SetQuot[\List X][\DRelator\,(=)]$.
\begin{align*}
  \FMSet X
    \DefEq
    \sum (\Var{n} : ℕ) \Where
...
In the proof of Theorem~\ref{thm:InjPresSurjective} we employ the fact that the type of finite bags $\FMSet X$, for some specific choice of $X$, is linearly-ordered and $\SetQuot[(\Fin n \to X)][\sim]$ is a definable set-quotient.
...
We analyse two constructions of the final coalgebra for the finite bag functor: as an $\omega$-limit %(Section~\ref{sec:final-limit-set})
...
%%   \]
  ...
  Using the latter equality, the definition of $\pres{\M}$ and the assumption $\Op{diag}$, we can form the following sequence of equalities:%\todo{better wording}
  \begin{align*}
