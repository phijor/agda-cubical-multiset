...
\item The groupoid-based polynomial functor $\Bag$ admits a final coalgebra $\Lim{\Bag}$ as the $\omega$-limit of (\ref{eq:chain}), a result arising directly from the work of Ahrens et al.~\cite{Ahrens2015}. $\Lim{\Bag}$ is a groupoid, not a set. One might wonder if the set-truncation $\SetTrunc{\Lim{\Bag}}$ is a good candidate for the final $\FMSet$-coalgebra. We  show that it is a fixpoint of $\FMSet$, but showing that it is the final coalgebra seems to  require a non-constructive principle similar to the axiom of choice. 
\end{enumerate}
...
The combination of Theorem~\ref{thm:FMSetOfFMGpdTrunc} and Theorem~\ref{prop:BagTote}, together with the fact that set truncation preserve equivalences, implies that $\SetTrunc{\Bag^n 1}$ is equivalent to $\FMSet^n 1$ for all $n$. One might wonder whether the set truncation of $\Lim{\Bag}$ delivers the final coalgebra of $\FMSet$. We are able to show that $\SetTrunc{\Lim{\Bag}}$ is a fixpoint of $\FMSet$, but the proof of finality requires an additional assumption, which shares some similarities with the axiom of choice.
...
Let $\theta_X : \SetTrunc{X \to \Bag X} \to (X \to \SetTrunc{\Bag X})$ be the canonical map defined by $\Rec{\SetTrunc{\Blank}}$. Generally there is no way to construct a map in the reverse direction. Assuming that such map exists and it is a section for $\theta_X$, we are able to prove the finality of the coalgebra underlying the equivalence in \cref{thm:FMSetFixpointOfTrunc}.
