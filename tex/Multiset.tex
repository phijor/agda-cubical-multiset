...
\item The groupoid-based polynomial functor $\Bag$ admits a final coalgebra $\Lim{\Bag}$ as the $\omega$-limit of (\ref{eq:chain}), a result arising directly from the work of Ahrens et al.~\cite{Ahrens2015}. $\Lim{\Bag}$ is a groupoid, not a set. One might wonder if the set-truncation $\SetTrunc{\Lim{\Bag}}$ is a good candidate for the final $\FMSet$-coalgebra. We  show that it is a fixpoint of $\FMSet$, but showing that it is the final coalgebra seems to  require the assumption of choice-like principles. 
\end{enumerate}
...
\begin{theorem}\label{thm:FMGpdLim}
  The function $\operatorname{pres} : \Bag\,\Lim{\Bag} \to \LimSh{\Bag}$ is an equivalence of groupoids. In particular, $\Lim{\Bag}$ is the final $\Bag$-coalgebra.
\end{theorem}

The combination of Theorem~\ref{thm:FMSetOfFMGpdTrunc} and Theorem~\ref{prop:BagTote}, together with the fact that set truncation preserve equivalences, implies that $\SetTrunc{\Bag^n 1}$ is equivalent to $\FMSet^n 1$ for all $n$. One might wonder whether the set truncation of $\Lim{\Bag}$ delivers the final coalgebra of $\FMSet$. We are able to show that $\SetTrunc{\Lim{\Bag}}$ is a fixpoint of $\FMSet$.
%but the proof of finality requires an additional assumption, which shares some similarities with the axiom of choice.
...
%(or, equivalently, a function $c' : X \to \SetTrunc{\Bag X}$)
...
%%   Assuming that $\theta_X$ has a section for all types $X$, then $\SetTrunc{\Lim{\Bag}}$ is the final coalgebra of $\FMSet$ in sets.
%% \end{theorem}
%% \begin{proof}
%% Let $c : X \to \FMSet X$ be a coalgebra, which is equivalent to having a function $c' : X \to \SetTrunc{\Bag X}$. We use the section of $\theta_X$ on $c'$ to build  an set-truncated $\Bag$-coalgebra $c'' : \SetTrunc{X \to \Bag X}$. Invoking $\Rec{\SetTrunc{\Blank}}$ on $c''$, we receive $g : X \to \Bag X$. From the finality of $\Lim{\Bag}$ in groupoids, there exists a unique coalgebra morphism $f^* : X \to \Lim{\Bag}$ and we define $f\,x \DefEq \SetTruncCon[f^*\,x]$. The function $f$ is the desired unique coalgebra morphism between $X$ and $\SetTrunc{\Lim{\Bag}}$.
%% \end{proof}
...
Similarly to the quotient $\SetQuot[\Lim{\List}][\Bisim]$ of Section~\ref{sec:final-quotient-set}, $\SetTrunc{\Lim{\Bag}}$ can be used to give semantics to transition systems with finite set of states: a section for $\theta_{X,Y}$ is derivable in case $X$ is a finite set using the \enquote{finite choice} principle in (\ref{eq:finite-choice}).
