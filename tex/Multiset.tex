...
%The proof of Theorem~\ref{thm:PresSplitEpi} relies on the validity of two specific instances of the axiom of countable choice. We show here that these instances hold and therefore Theorem~\ref{thm:PresSplitEpi} is true without the need of postulating countable choice. This is achieved by showing that, whenever a type $X$ is endowed with a linear order $(<)$, then $\SetQuot[\List X][\Perm]$ is a definable set-quotient.
...
\begin{theorem}[\CodeRef{Multiset.FCM.Limit}{pres-inj⇒complete}]\label{lem:InjPresImpliesComplete}
  If $\pres{\FMSet}$ is injective, then \LLPO{} holds.
  \todo[inline]{%
    Split this into the main theorem (injectivity implies completeness)
    and a corollary (also implies \LLPO{})
  }
\end{theorem}
\begin{proof}
...
\begin{theorem}[\CodeRef{Multiset.FMSet.Limit}{SplitEpimorphism.pres-section}]\label{thm:PresSplitEpi}
...
The setoid $(\Lim{\List},\Bisim)$ has a $\Op{FMSetoid}$-coalgebra structure which is final in the category of setoids.
  \todo[inline]{%
    We have formalized a proof that $\pres{\List}$ is \emph{full},
    but \emph{faithfulness} implies LLPO.
    The inverse should be similar to \cref{lem:LLPOImpliesPresInj}.
    Omit proof of finality.
  }
\end{theorem}
...
\begin{theorem}[\CodeRef{Multiset.ListQuotient.Finality}{fixQ⁺-iso}]\label{thm:fixpoint-quotient}
...
\begin{theorem}\label{thm:final-set-truncation}
  ...
  \todo{Specify $\operatorname{AC}_{x,y}$, for appropriate $x, y$. C.f. \cite[Ex.~7.8]{HoTTBook}}
\end{theorem}
\begin{proof}
...
\todo[inline]{Remove this section, adjust mentions of this section.}
