...
\todo[inline]{Link all equivalent definitions of finite multisets.}
...
\begin{theorem}[\CodeRef{Multiset.FCM.Limit}{pres-inj⇒complete}]\label{lem:InjPresImpliesComplete}
  ...
  \todo[inline]{%
    Merge \texttt{complete-to-llpo}, and reference that.
  }
\end{theorem}
\begin{proof}
...
\begin{theorem}[\CodeRef{Multiset.FMSet.Limit}{SplitEpimorphism.pres-section}]\label{thm:PresSplitEpi}
 The function $\pres{\FMSet}$ has a section.
  \todo{Get rid of all mentions of \enquote{split-epimorphism}, in the paper and the code.}
\end{theorem}
\begin{proof}
...
In order to obtain $\Bisim$, we first iterate the $\Relator$:\todo{motivate this definition}
\begin{align*}
...
\begin{theorem}
  Assuming $\coalg{\List}$ is a setoid-morphism, \LLPO{} holds.
  \todo{Link \texttt{fix⁻-preserves-≈→LLPO} once in}
\end{theorem}
...
\begin{theorem}[\CodeRef{Multiset.ListQuotient.Finality}{fixQ⁺-iso}]\label{thm:fixpoint-quotient}
  ...
  \todo{Make this an equivalence in the code.}
\end{theorem}
