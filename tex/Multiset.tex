...
The proof of \cref{thm:FMSetFixpointOfTrunc} employs the fact that $\FMSet$ is closed under set-truncation.  The latter fact factors through the following lemma, stating that set-truncation distributes over finite families of types.
\begin{lemma}[{\CodeRef{finChoiceEquiv}}]\label{lem:FiniteChoice}
...
\begin{proposition}[{\CodeRef{SymActDefinable}}]\label{prop:ListPermDefQuot}
...
\begin{proposition}[{\CodeRef{SymActUntruncate}}]\label{prop:ListPermCanonicalPerm}
...
\begin{proposition}[{\CodeRef{linLexFMSet}}]\label{prop:lift-linear-order}
...
This holds since naturally equivalent chains have equivalent limits (see \CodeRef{chainEquivToLimitEquiv} for details).
...
\begin{lemma}[\CodeRef{FMSetAlternationLemma}]\label{lem:DiagLimCaseAnalysis}
...
Nevertheless, we are able to salvage the fact that $\pres{\FMSet}$ has a section/right-inverse which targets the shifted limit. This means that $\FMSet$ weakly preserves the $\omega$-limit $\Lim{\FMSet}$, but strong limit-preservation is equivalent to \LLPO{}.
...
\end{lemma}
\begin{proof}
Define $(<^n) : \FMSet^n 1 \to \FMSet^n 1 \to \Type$ by induction on $n$: $(<^0)$ is the empty relation and $x <^{1 + n} y \DefEq \LexPerm\,(<^n)\;x\;y$. \Cref{prop:lift-linear-order} implies that the order $(<^n)$ is linear for all $n$.
\end{proof}
...
\begin{theorem}[\CodeRef{isSetoidMorphismCoalgListToLLPO}]
...
\begin{theorem}[\CodeRef{finalFMSetoidCoalgebra}]\label{thm:final-setoids}
...
\begin{theorem}[\CodeRef{FMSetFixpointTree/Bisim}]\label{thm:fixpoint-quotient}
...
\begin{theorem}[\CodeRef{FinalFMSetCoalgebra}]\label{thm:final-quotient}
...
%\paragraph{Comparing $\Op{List}$ and $\Bag$}\todo{Fit this comparison in somewhere}
  \begin{proposition}[\CodeRef{BagToteEquiv}]\label{prop:BagTote}
    For any type $X$, the equivalence of \cref{prop:BijFinSet} extends to an equivalence $\Bag X \simeq \Tote X$.
  \end{proposition}
...
\begin{corollary}[Theorem 7 of \cite{Ahrens2015}]\label{cor:BagFinalCoalg}
...
\end{theorem}
\begin{proof}
Let $c : X \to \FMSet X$ be a coalgebra, which by \cref{thm:FMSetOfFMGpdTrunc} and \cref{prop:BagTote} is equivalent to having a function $c' : X \to \SetTrunc{\Bag X}$. Applying $\mathsf{AC}_{3,2}$ on $c'$ gives $c'' : \SetTrunc{X \to \Bag X}$. Invoking $\Rec{\SetTrunc{X \to \Bag X}}$ on $c''$, we receive $g : X \to \Bag X$. From the finality of $\Lim{\Bag}$ in \cref{cor:BagFinalCoalg}, there exists a unique coalgebra morphism $f^* : X \to \Lim{\Bag}$ and we define $f\,x \DefEq \SetTruncCon[f^*\,x]$. The function $f$ is the desired unique coalgebra morphism between $(X,c)$ and $(\SetTrunc{\Lim{\Bag}},\coalg{\FMSet})$. The proof of uniqueness uses an application of the axiom of choice. We refer to the formalization for details.
\end{proof}
...
But we know how to prove the surjectivity of $\pres{\Op{F}}$ under the assumption of the axiom of countable choice.
...
\]
The latter can be related to $\Op{F}_{A,B,P}$ similarly to how $\Bag$ and $\FMSet$ are related via \cref{thm:FMSetOfFMGpdTrunc,prop:BagTote}: $\SetTrunc{\Op{G}_{A,B,P} X} \simeq \Op{F}_{A,B,P} X$. Since $\Op{G}_{A,B,P}$ is a polynomial functor, its final coalgebra can be constructed as an $\omega$-limit, as in \cref{thm:BagPresLim,cor:BagFinalCoalg}.
%% \begin{itemize}
