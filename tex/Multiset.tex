...
Finally, we introduce the type of finite bags over $X$ as an \emph{analytic functor}~\cite{Joyal1986}, which is provably equivalent to $\M X$, $\SetQuot[\List X][\Perm]$ and $\SetQuot[\List X][\Relator\, (=)]$.
\begin{align*}
  \FMSet X
    \DefEq
    \sum (\Var{n} : ℕ) \Where
...
Nevertheless, we are able to salvage in our constructive setting the fact that $\pres{\FMSet}$ is a split epimorphism, i.e. it has a section/right-inverse. This means that $\FMSet$ weakly preserves the $\omega$-limit $\Lim{\FMSet}$, but strong limit-preservation is equivalent to \LLPO{}.
...
\begin{theorem}\label{thm:final-quotient}
  Assuming the axiom of choice, $\SetQuot[\Lim{\List}][\Bisim]$ is the final coalgebra of $\FMSet$.
\end{theorem}
...
and constructing the final coalgebra of the latter in a straighforward way %(\cref{ssec:FMGpdLim},
(\cref{thm:FMGpdLim}).
...
%    \small
  ...
  \end{proposition}
  A small type of finite bags valued in groupoids is defined by replacing $\Op{FinSet}$ with $\Bij$.
...
We employ an alternative formulation of the axiom of choice, provably equivalent to more standard formulations~\cite{Veltri2021}. Given types $X,Y$ and a relation $R$ on $Y$, define the relation $X \to R$ on $X \to Y$ as the pointwise lifing of $R$ to functions. The axiom of choice states that the function $\theta_R : \SetQuot[(X \to Y)][(X \to R)] \to (X \to \SetQuot[Y][R])$, definable via $\Rec{\SetQuot[X \to Y][X \to R]}$, has a section. From the fact that $e : (\SetQuot[X][{=}]) \simeq \SetTrunc{X}$, the axiom of choice gives in particular a function $\psi : (X \to \SetTrunc{Y}) \to \SetTrunc{X \to Y}$ which is a right-inverse of $\theta_{=}$ (up to equivalence $e$).
\begin{theorem}\label{thm:final-set-truncation}
  Assuming the axiom of choice, $\SetTrunc{\Lim{\Bag}}$ is the final coalgebra of $\FMSet$.
\end{theorem}
\begin{proof}
...
We now look at an alternative construction of the final coalgebra using Cubical Agda's coinductive types \cite{Vezzosi2019} and a generalization of our constructions to other analytic functors \cite{Joyal1986}.
...
When $B$ is valued in sets, one can also define a function $\langle -\rangle : \Op{U}_{A,B,P} \to \Type$ extracting a set, which for $\Obj a$ is $B\,a$.
...
Our conclusion is in line with the one of Kock~\cite{Kock2012} and Finster et al.~\cite{Finster2021}: the bag functor is better behaved when valued in groupoids instead of sets, especially from the perspective of final semantics. This means that the denotational semantics of resource-sensitive computations %or dynamical systems
