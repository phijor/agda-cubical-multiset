...
The material presented in the paper (apart from Section~\ref{sec:analytic}) have been formalized in the Cubical Agda proof
...
\end{proof}
%\begin{definition}
The equivalence of Lemma \ref{lem:FiniteChoice} allows to define a variant of the elimination principle $\Elim{\SetTrunc{X}}$ which takes $\Fin n \to \SetTrunc{X}$ as input (a sort of ``finite choice'' principle for set truncation)
%taking The principle of finite choice can be defined in terms of $\Op{box}$:
  \begin{equation}\label{eq:finite-choice}
    ...
    \FinElim : \{n : ℕ\}\, \{B : (\Fin n \to \SetTrunc{X}) \to \Type\} \to (\forall v\Where \IsSet (B\, v)) \\
      \hspace{1.5cm}\to ((w : \Fin n \to X) \to B\, (\SetTruncCon{} \circ w)) \\
...
Consider the chain in (\ref{eq:chain}), with $F$ replaced by $\FMSet$. We formally define $\FMSet^n 1$ by recursion on $n$: $\FMSet^0 1 \DefEq 1$ and $\FMSet^{1 + n} 1 \DefEq \FMSet (\FMSet^n 1)$. Similarly we can define the iteration $\Bang^n_{\FMSet} = \Map{\FMSet}^n\,\Bang$.
...
Both injectivity of $\pres{\FMSet}$ and \LLPO{} are propositions, so it is sufficient to show that there is a bi-implication between them. But first a useful lemma.
...
\begin{proof}
  ...
  It is sufficent to show that the injectivity of $\pres{\M}$ implies that the following type is inhabited:
...
%%   \]
  ...
  Using the latter equality, the definition of $\pres{\M}$ and the assumption $\Op{diag}$, we can form the following sequence of paths:%\todo{better wording}
  \begin{align*}
  ...
  \end{align*}
  which implies that $\pres{\M}\{x, \overline{x}\} = \pres{\M}\{y_1, y_2\}$.
...
%  \todo[inline]{Prove it.}
%\end{proof}
For the proof, which employs the functor $F X \DefEq \SetQuot[\List X][\Relator\,(=)]$ instead of $\FMSet$, we refer the reader to our Agda formalization.  The proof is similar to the one of a related result in \cite{Veltri2021}: the injectivity of $\pres{\Op{Pfin}} : \Op{Pfin} \,\Lim{\Op{Pfin}} \to \LimSh{\Op{Pfin}}$, where $\Op{Pfin}$ is the finite powerset functor, is derivable from \LLPO{} and the axiom of countable choice. It turns out that countable choice is not needed, neither in \cref{lem:LLPOImpliesPresInj} nor in the similar result for the finite powerset functor.

Nevertheless, we are able to salvage in our constructive setting that $\pres{\FMSet}$ is a split epimorphism, i.e. it has a section. This means that $\FMSet$ weakly preserves the $\omega$-limit $\Lim{\FMSet}$, but stong limit-preservation is equivalent to \LLPO{}.
...
\begin{theorem}\label{thm:InjPresSurjective}
 The function $\pres{\FMSet}$ is a split epimorphism.
\end{theorem}
\begin{proof}
...
\begin{proof}
  ...
  A function $\ToTote : \FMSet X → \SetTrunc{\Tote X}$ can be defined by
  ...
  by invoking $\Rec{\SetTrunc{\Tote X}}$, so it is enough to provide $g : \Tote{X} → \FMSet X$. %which is given as $g\, ((B, \Var{n}, e), v) \DefEq (\Var{n} , g')$.
...
Following \cite{Finster2021}, we first introduce an equivalent but small definition $\Bij$ of the type of finite sets $\Op{FinSet}$.% that avoids this issue.
  \begin{center}
%    \small
    \hspace*{\fill}
      \AxiomC{$n : \N$}
...
We believe that $\Bag X$ can be proved equivalent to the \emph{free symmetric monoidal groupoid} on $X$ defined as a HIT by Piceghello \cite{Piceghello2021}. The equivalence would serve as an alternative proof of MacLane's coherence for symmetric monoidal categories.
...
\begin{proof}
  ...
  \cref{thm:FMSetFixpointOfTrunc:step2} follows from Theorems~\ref{thm:FMSetOfFMGpdTrunc} and \ref{prop:BagTote},
  and \eqref{thm:FMSetFixpointOfTrunc:step3} follows from \cref{thm:FMGpdLim}.
  %since $\Lim{\Bag}$ is a limit.
\end{proof}
%It seems highely unlikely that the coalgebra underlying the equivalence in \cref{thm:FMSetFixpointOfTrunc} is provably final without the assumption of any choice-like principles. Initial investigations show that, in order to construct a coalgebra morphism typed $X \to \SetTrunc{\Lim{\Bag}}$ from a coalgebra $c : X \to \FMSet X$, we need to assume the following principle: the function $\theta_{X,Y} : \SetTrunc{X \to Y} \to (X \to \SetTrunc{Y})$, which can be easily defined using  $\Rec{\SetTrunc{\Blank}}$, has a section for all types $X$ and $Y$. The proof of uniqueness of the mediating coalgebra morphism seems to additionally require an instance of the axiom of choice. We refrain from showing a proof of finality here, which we leave to future work. We do not know what is the strength of assuming the existence of a section for $\theta_{X,Y}$, nor how it compares to other choice principles. It surely resembles a form of choice: if set truncation is replaced with propositonal truncation, then the required principle becomes a particular instance of the axiom of choice.
We employ an alternative formulation of the axiom of choice, provably equivalent to more standard formulations~\cite{Veltri2021}. Given types $X,Y$ and a relation $R$ on $Y$, define the relation $X \to R$ on $X \to Y$ as the pointwise lifing of $R$. The axiom of choice states that the function $\theta_R : \SetQuot[(X \to Y)][(X \to R)] \to (X \to \SetQuot[Y][R])$, definable via $\Rec{\SetQuot[X \to Y][X \to R]}$, has a section. From the existence of an equivalence $e : (\SetQuot[X][{=}]) \simeq \SetTrunc{X}$, choice gives in particular a function $\psi : (X \to \SetTrunc{Y}) \to \SetTrunc{X \to Y}$ which is a right-inverse of $\theta_{=}$ (up to equivalence $e$).
\begin{theorem}\label{thm:final-set-truncation}
  Assuming the axiom of choice, $\SetTrunc{\Lim{\Bag}}$ is the final coalgebra of $\FMSet$ in sets.
\end{theorem}
\begin{proof}
Let $c : X \to \FMSet X$ be a coalgebra, which is equivalent to having a function $c' : X \to \SetTrunc{\Bag X}$. Applying $\psi$ on $c'$, we a set-truncated $\Bag$-coalgebra $c'' : \SetTrunc{X \to \Bag X}$. Invoking $\Rec{\SetTrunc{X \to \Bag X}}$ on $c''$, we receive $g : X \to \Bag X$. From the finality of $\Lim{\Bag}$ in groupoids, there exists a unique coalgebra morphism $f^* : X \to \Lim{\Bag}$ and we define $f\,x \DefEq \SetTruncCon[f^*\,x]$. The function $f$ is the desired unique coalgebra morphism between $X$ and $\SetTrunc{\Lim{\Bag}}$.
\end{proof}
...
We conclude the paper by looking at the final coalgebra of the bag functor using Cubical Agda's coinductive types \cite{Vezzosi2019} and generalizations of our constructions to other analytic functors \cite{Joyal1986}.
...
The coinductive type associated to $\M$ is given by the following coinductive record type:
\[
...
\end{array}
\]
The type $\cLim{\M}$ has a constructor $\fold : \cLim{\M}\to \M \cLim{\M}$ and a destructor 
$\unfold : \cLim{\M} \to \M \cLim{\M}$ which are mutual inverses. In particular, $\unfold \circ \fold$ is judgementally equal to the identity function, while the other equations can be proved by corecursion and path introduction:
\[
\def\arraystretch{0.8}
\begin{array}{l}
...
\end{array}
\end{equation*}
Notice that Agda's strict productivity checker actually rejects this definition, and practically in our formalization we employ \emph{sized types}~\cite{Hughes1996,Danielsson2018} to work around these issues.
%\end{array}
%\end{equation*}
...
The formulation $\FMSet$ of the finite bag functor exposes this as an analytic functor, which differs from a polynomial functor in that the tuple of elements $\Fin\,n \to X$ is quotiented by the relation generated by the action of the symmetric group with $n$ elements on $X$. Other analytic functors arise by choosing a different subgroup of the symmetric group. For example, picking the subgroup of cyclic permutations delivers the functor of cyclic lists, while taking the trivial subgroup allows us to recover the list functor. 

More generally analytic functors can be seen as instances of the functors associated to the \emph{quotient containers} of Abbott et al.~\cite{Abbott2004}. A quotient container is a triple consisting of a type $A$, a type family $B : A \to \Type$ and a propositionally-valued type family $P : \forall \{a\} \Where B \,a \simeq B \,a \to \Type$ which is closed under identity, inverses and composition of equivalences. The associated functor is:
\[
...
\Op{Act} \,P\,a\;v\;w \DefEq 
    ∃ (\sigma : B\,a ≃ B\,a) \Where P\,\sigma \times
      (v = w \circ \sigma)
\]
The type $\Op{F}_{A,B,P}\,X$ is a set whenever the type of shapes $A$ is a set. The functor $\FMSet$ is the instance where $A \DefEq ℕ$, $B \DefEq \Fin$ and $P\,\sigma \DefEq 1$.

We know that the construction of the final coalgebra as an $\omega$-limit in sets for a general analytic functor $\Op{F}_{A,B,P}$ is problematic, since it is already problematic for $\FMSet$. Nevertheless, one can ask if a result like Theorem~\ref{thm:InjPresSurjective} is valid for any $\Op{F}_{A,B,P}$. The answer is no: the function $\pres{\Op{F}} : \Op{F}_{A,B,P} (\lim_{n} (\Op{F}_{A,B,P}^n 1)) \to \lim_{n} (\Op{F}_{A,B,P}^{1 + n} 1)$ is not generally a split epimorphism, and it can be shown to be surjective only in the presence of the axiom of countable choice. In fact, the proof of Theorem~\ref{thm:InjPresSurjective} relies on Propositions~\ref{prop:ListPermDefQuot} and \ref{prop:ListPermCanonicalPerm}, which are very specific properties of the finite bag functor. 
The employment of these propositions can be seen as the invocation of two specific instances of the axiom of countable choice.

Each quotient container $(A,B,P)$ also specifies a polynomial functor $\Op{G}_{A,B,P}$ in groupoids, akin to the functor $\Bag$. First, the small HIT construction of the groupoid of finite types $\Bij$ can be generalized:
  \begin{center}
    \hspace*{\fill}
      \AxiomC{$a : A$}
      \UnaryInfC{
    ...
    \hfill
      \AxiomC{$a : A$}
      \UnaryInfC{$\Hom \,\mathsf{id} \Op{Prefl}= \Refl\,(\Obj a)$}
      \DisplayProof
    \hspace*{\fill}
    \\[1em]
    \hspace*{\fill}
    ...
    \hspace*{\fill}
  \end{center}
Here $\Op{Prefl}$ and $\Op{Pcomp}$ are the closures of $P$ wrt.\@ identity and composition of equivalences.
When $B$ is valued in sets, one can also define a function $\langle\Blank\rangle : \Op{U}_{A,B,P} \to \Type$ extracting a type, which for $\Obj a$ is $B\,a$.
...
We looked at various definitions of the finite bag functor, in sets and in groupoids, and constructions of their final coalgebras. When working with set-based definitions, the set-theoretic constructions as $\omega$-limit of the chain (\ref{eq:chain}) and as quotient of the final $\List$-coalgebra are not replicable in HoTT without the assumption of classical principles such as LLPO or the axiom of choice. We are at least able to salvage the \emph{weak} preservation of the $\omega$-limit in (\ref{eq:chain}). The situation is brighter when working with the groupoid-based definition. The latter is a polynomial functor, and as such has the final coalgebra given by the $\omega$-limit in (\ref{eq:chain}). We briefly looked at an application of Cubical Agda's coinductive types for building the final coalgebra in sets, and generalizations to other analytic functors.

Our conclusion is in line with the one of Kock~\cite{Kock2012} and Finster et al.~\cite{Finster2021}: the bag functor is better behaved when defined in groupoids instead of sets, especially from the perspective of final semantics. This means that the denotational semantics of resource-sensitive computations or dynamical systems is better performed using groupoids instead of sets (switching from categorical to bicategorical semantics). In particular, the specification of resource-sensitive programming languages could be given directly as a groupoid, e.g. structural congruences could become data and not property. We plan to investigate this connection to programming language semantics in future work.

The construction of the final coalgebra in Section~\ref{sec:coinductive} uses a HIT in the codomain
of a coinductive type destructor. This should be deemed an experimental feature of Cubical Agda, since the interaction of coinductive types and HITs has not yet been investigated (only for some M-types~\cite{Vezzosi2019}, which are definable internally anyway). Recent work by Kristensen et al.~\cite{Kristensen2022} seems to indicate that final coalgebras of functors such as $\M$ or $\Op{Pfin}$, which are defined as HITs, should be definable as \emph{strict} $\omega$-limit of the chain in (\ref{eq:chain}) in the cubical set model. Strictness means that the limit-property holds on the nose, not only up-to path equality. This would also be validated by the situation in setoids (\cref{thm:final-setoids}).
