...
\begin{proof}
  ...
  For each $d : ℕ$, we know that $\ell_{d + 1}\,s$ is path equal to a pair of the form $(n*,v_d)$. In order to construct $u$ we need access to a representative of the equivalence class $v_d : \SetQuot[\Fin n^* \to \FMSet^d 1][\sim]$ for each $d$. We know that this can be done using Lemma~\ref{lem:iter-linear} and Proposition~\ref{prop:ListPermDefQuot}. Let $w_d : \Fin n^* \to \FMSet^d 1$ be the canonical representative of $v_d$. The limit-property of $s$ can be translated to the mere existence of a permutation $\sigma_d : \Fin n^* \simeq \Fin n^*$ such that $\Map{\FMSet}^d\,!\,(w_{1 + d} \,(\sigma_d\,k)) = w_d\,k$, for all $d$ and $k : \Fin n^*$. The construction of $u$ also requires access to the each permutation $\sigma_d$, that sits inside a propositional truncation for each $d$. We can access this by invoking Lemma~\ref{lem:iter-linear} and Proposition~\ref{prop:ListPermCanonicalPerm}.

  We now have all the ingredients for building $u$. Define a permutation $\sigma^*_d : \Fin n^* \simeq \Fin n^*$ by induction on $d$: $\sigma^*_0 \DefEq \Op{id}$, $\sigma^*_{1+d} \DefEq \sigma_d \circ \sigma^*_d$. Then take $u$ such that $\ell_d \,(u\,k) \DefEq w_d\,(\sigma^*_d \,k)$. One can show that $u\,k : \Lim{\FMSet}$ for all $k : \Fin n^*$ and that $t$ is indeed in the fiber of $\Op{pres}$ over $s$.
\end{proof}
