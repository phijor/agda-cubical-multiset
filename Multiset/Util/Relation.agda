...
module _ (S : Rel B B ℓS) (R : Rel A A ℓR) where
  ReflectsRel : (f : A → B) → Type _
  ReflectsRel f = ∀ {a a'} → S (f a) (f a') → R a a'

ReflectsRelComp : ∀ (R : Rel A A ℓR) (S : Rel B B ℓS) (T : Rel C C ℓT) {f : A → B} {g : B → C} → ReflectsRel S R f → ReflectsRel T S g → ReflectsRel T R (g ∘ f)
ReflectsRelComp _ _ _ f-reflects g-reflects = f-reflects ∘ g-reflects


module _ (R : Rel A A ℓR) (S : Rel B B ℓS) where
  ReflectsRel→SectionPreservesRel : (f : A → B) (g : B → A) → (section f g) → ReflectsRel S R f → PreservesRel S R g
  ...
  PreservesRel→SectionReflectsRel : (f : A → B) (g : B → A) → section f g → PreservesRel R S f → ReflectsRel R S g
