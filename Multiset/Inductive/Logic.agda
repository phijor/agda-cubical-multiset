...
open import Multiset.Util.Logic
...
∈-map-fiber : ∀ {f : X → Y} {y : Y} {xs : M X}
  → y ∈ map f xs
  → ⟨ ∃[ x ] x ∈ₚ xs ⊓ y ≡ₚ f x ⟩
∈-map-fiber {f = f} {y} {xs} = ind {P = λ xs → y ∈ map f xs → ⟨ (∃[]-syntax (λ x → x ∈ₚ xs ⊓ y ≡ₚ f x)) ⟩}
...
⊕≡ε→ε⊔ε {X = X} {xs = xs} {ys = ys} = M.ind {P = λ xs → ∀ ys → xs ⊕ ys ≡ ε → ⟨ xs ≡ₚ ε ⊔ ys ≡ₚ ε ⟩}
  (λ xs → isPropΠ2 λ ys h → isProp⟨⟩ _)
  empty* singl* union* xs ys where
    empty* : ∀ ys → ε ⊕ ys ≡ ε → ⟨ _ ⊔ ys ≡ₚ ε ⟩
    empty* ys indH = ⊔-inr ∣ subst (_≡ ε) (unit ys) indH ∣₁

    singl* : ∀ (x : X) (ys : M X) → η x ⊕ ys ≡ ε → ⟨ _ ⊔ _ ⟩
